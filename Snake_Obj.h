...
Quad *pQuad;
	 ...
	 void SetDepthSnake(const float depth);
	 float GetDepthSnake() const;
	 int GetIndexTail() const; // где у нас находится хвост в структуре s[20]
	 int GetDir() const;
	 int GetHeadPosX() const;
	 int GetHeadPosY() const;
     int GetPosTailX() const;
	 int GetPosTailY() const;
