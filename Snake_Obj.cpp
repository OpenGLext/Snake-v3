...
int Snake_Obj::GetHeadPosX() const
{
	return s[0].x;
}

int Snake_Obj::GetHeadPosY() const
{
	return s[0].y;
}

int Snake_Obj::GetPosTailX() const
{
	return s[m_len_body].x;
}

int Snake_Obj::GetPosTailY() const
{
	return s[m_len_body].y;
}
...
int Snake_Obj::GetIndexTail() const
{
	for (int i=0; i<m_len_body; i++)
	{
		if ( s[i].x != 0 && s[i].y != 0 ) { return i; }
	}
}

  void Snake_Obj::SetDepthSnake(const float depth)
  {
	  this->depthSnake = depth;
  }
  float Snake_Obj::GetDepthSnake() const
   {
	   return this->depthSnake;
...
if ((s[0].x == m_fruct_x[0]) && (s[0].y) == m_fruct_y[0]) { m_len_body++; m_IsCollision = true; PointFruct.SetVisible(false,0); return true;   } //
else {}
