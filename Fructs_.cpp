...
int Fructs_::GetPosXFruct() const
{
	return m_vec[0].x;
}

int Fructs_::GetPosYFruct() const
{
	return m_vec[0].y;
}
...
void Fructs_::DrawApple()
{
  ...
  // }
   ...
   if ( visibleFructs[0] )
   {
	   this->box_(m_vec[0].x * m_scale, m_vec[0].y * m_scale, (m_vec[0].x + frame) * m_scale, (m_vec[0].y + frame) * m_scale);
   }
